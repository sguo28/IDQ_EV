...
from config.hex_setting import OFF_DURATION,DIM_OF_RELOCATION,DIM_OF_CHARGING
...
class hex_zone:
    ...
    def dispatch(self,vehicles,current_time):
        ...
        if len(vehicles.keys()) == 0:
            return []
        commands = self.get_dispatch_decisions(vehicles)
        # perform dispatch
        od_pairs = []
        dispatched_veh_list = []
        new_commands =[]
        ...
        todo: [done]some destination is not in command
        '''
        # print([veh for veh in vehicles.values()])
        for command,dispatched_veh in zip(commands, vehicles.values()):
            ...
            if "offduty" in command:
                ...
                o_lonlat, d_lonlat = command["cache"]
                ...
                if command['charge_flag'] ==0: # dispatch to relocate
                    ...
                    c_id = self.nearest_cs[command['action']-DIM_OF_RELOCATION ]
                    charging_station_repo = ChargingRepository.get_charging_station(c_id)
                    dispatched_veh.head_for_charging_station(triptime,charging_station_repo, route)
            else: #if 'destination' in command:
        ...
        for dispatched_veh, (route, triptime),command in zip(dispatched_veh_list, routes,new_commands):
            # if triptime == 0:
            #     continue
            if command['charge_flag'] ==0: # dispatch to relocate
                dispatched_veh.cruise(route, triptime,command['destination_hex_id'],command['action'],current_time)
    ...
    def get_dispatch_decisions(self, tbd_vehicles):
        dispatch_commands = []
        # a_ids, offduties = self.predict_best_actions(tbd_vehicles)
        for vehicle in tbd_vehicles.values():
            # for test
            # vehicle.state.dispatch_action_id = [0]
            # Get best action for this vehicle and whether it will be offduty or not
            if len(vehicle.state.dispatch_action_id)==0:
                command =[]
            else:
                a_id, offduty = vehicle.state.dispatch_action_id.pop(),0 # actions are attached before implementing dispatch
                if offduty:
                    command = self.create_dispatch_dict(vehicle_id=vehicle.state.vehicle_id, offduty=True,action=a_id, charge_flag = 0)
                else:
                    # Get target destination and key to cache
                    target, charge_flag, target_hex_id = self.convert_action_to_destination(vehicle, a_id)
                    # create dispatch dictionary with the given attribute
                    if target is None:
                        continue
                    if cache_key is None:
                        command = self.create_dispatch_dict(vehicle_id= vehicle.state.vehicle_id, destination= target,action=a_id, charge_flag =charge_flag, dest_hex_id =target_hex_id)
    ...
    def convert_action_to_destination(self, vehicle, a_id):
        '''
        :to do: [done] 7 is dim of relocation action space
        '''
        # cache_key = None
        target = None
        ...
        try:
            ...
            lon,lat = self.cs_loc.loc[self.nearest_cs[a_id-DIM_OF_RELOCATION],['Longitude','Latitude']].values
            vehicle.state.status = status_codes.V_WAYTOCHARGE
            charge_flag = 1
        target = (lon,lat)
        # if lon == vehicle.state.lon and lat == vehicle.state.lat:
        #     pass
        # if FLAGS.use_osrm and (vehicle.state.lon, vehicle.state.lat) == (lon, lat): # use OSRM engine to generate inter-zone trips
    ...
    def create_dispatch_dict(self, vehicle_id, destination=None, offduty=False, cache_key=None,action = None, charge_flag = 0, dest_hex_id =0):

        dispatch_dict = {}
        dispatch_dict["vehicle_id"] = vehicle_id
        dispatch_dict["action"] =action # id
