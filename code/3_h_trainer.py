...
from config.hex_setting import HEX_SHP_PATH, CS_SHP_PATH, NUM_NEAREST_CS, TRIP_FILE, TRAVEL_TIME_FILE, \
    TIMESTEP, START_OFFSET, SIM_DAYS,  START_TIME, TRAINING_CYCLE, STORE_TRANSITION_CYCLE
...
if __name__ == '__main__':
    ...
    if SIM_DAYS > 0:
        ...
        simulator.init(HEX_SHP_PATH, CS_SHP_PATH, TRIP_FILE, TRAVEL_TIME_FILE, NUM_OPTION,TRAINING_F,NUM_NEAREST_CS)

        dqn_agent = DeepQNetworkAgent(simulator.hex_diffusions, args.isoption, args.islocal, args.ischarging)
        h_agent = H_Agent(simulator.hex_diffusions,NUM_OPTION, args.isoption, args.islocal, args.ischarging)
        ...
        with open('logs/parsed_results_%s_%s_%s.csv' % (isoption, islocal, ischarging), 'w') as f, open(
            ...
            for episode in range(NUM_EPISODE): # we need at least 2e4 steps = ~ 14 days.
                ...
                for day in range(SIM_DAYS):
                    ...
                    for i in range(n_steps):
                        ...
                        if len(local_state_batches) > 0:
                            selected_actions,action_to_execute,new_assigned_opts, contd_options = dqn_agent.get_actions(local_state_batches, num_valid_relos, global_state,assigned_option_ids)
                            simulator.attach_actions_to_vehs(selected_actions,action_to_execute,new_assigned_opts, contd_options)
                        ...
                        if tick % STORE_TRANSITION_CYCLE == 0:
                            ...
                            if states is not None:
                                [h_agent.add_transition(states, actions, next_states, trip_flag,
                                                          time_steps, valid_action_num_) for
                                 states, actions, next_states, trip_flag, time_steps, valid_action_num_ in
                ...
                states, actions, next_states, rewards, terminate_flags, time_steps, valid_action_nums_ = simulator.dump_prime_action_to_dqn()
                if states is not None:
                    for state, action, next_state, reward, terminate_flag, time_step, valid_action_num_ in zip(
                            states, actions, next_states, terminate_flags, time_steps, valid_action_nums_):
                        h_agent.add_transition(state, action, next_state,  terminate_flag, time_step,
                                                valid_action_num_)
