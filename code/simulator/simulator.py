...
from config.hex_setting import FLAGS, NUM_REACHABLE_HEX, NUM_NEAREST_CS, ENTERING_TIME_BUFFER, charging_station_data_path, hex_route_file
import copy
import pickle
import ray
...
class Simulator(object):
        ...
        with open(file_name, 'r') as f:
            ...
            for lines in f:
                ...
                self.charging_station_collections.append(
                    charging_station(n_l2=int(float(num_l2)), n_dcfast=int(float(num_dc)), lat=ilat, lon=ilon,
                                     hex_id=hex_id))
    ...
    def init(self, file_hex, file_charging, trip_file, travel_time_file, n_nearest=NUM_NEAREST_CS):
        ...
        [self.match_zone_collection.append(matching_zone.remote(idx, hexs, self.hex_routes[:,:,1:2])) for idx, hexs in
    ...
    def par_step(self):  # we use parallel update to call the step function.
        ...
        self.vehicle_update(timestep,tick)
        ...
        print('Iteration {} completed, total cpu time={}, time for vehicle status update={}'.format(tick / 60, t_end,
                                                                                                    t_v_update))
    ...
    def download_match_zone_metrics(self):
        total_matches = ray.get([m.get_num_of_matches() for m in self.match_zone_collection])
        self.num_match = sum(total_matches)
    ...
    def update_vehicles(self):
        ...
        for hex in self.hex_zone_collection.values():
            ...
            [vehicle.update_info(self.__dt, self.__t, hex,self.route_dict) for vehicle in hex.vehicles.values()]
    ...
    def vehicle_update(self,timestep,timetick):
        [m.update_vehicles.remote(timestep,timetick) for m in self.match_zone_collection]
    ...
    def dump_transition_batch(self):
        return self.transition_batch.pop()
