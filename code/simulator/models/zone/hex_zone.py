...
from config.hex_setting import OFF_DURATION, DIM_OF_RELOCATION,FLAGS
...
class hex_zone:
    def __init__(self, hex_id, coord, coord_list, match_zone, neighbors, charging_station_ids, charging_hexes,
    ...
    def demand_generation(self, tick):  # the arrival of passenger demand
        ...
        with local_seed(tick):  # fix the random seed
            ...
            if narrivals > 0 and sum(destination_rate) > 0:
                ...
                destinations = np.random.choice(np.arange(destination_rate.shape[-1]), p=destination_rate,
        ...
        with local_seed(tick):  # fix the random seed
            ...
            if narrivals > 0 and sum(destination_rate) > 0:
                ...
                for i in range(narrivals):
                    ...
                    #        'trip_time':self.trip_time[hour,destinations[i]],'request_time':tick}
                    # r=request(self.total_pass, self.hex_id, (self.lon,self.lat,), destinations[i], self.coord_list[destinations[i]],self.trip_time[hour,destinations[i]],tick)
                    newpass[(self.hex_id, self.total_pass)] = Customer(
                        request(self.total_pass, self.hex_id, (self.lon, self.lat,), destinations[i],
                                self.coord_list[destinations[i]], self.trip_time[hour, destinations[i]],
                                tick))  # hex_id and pass_id create a unique passenger identifier
                    self.total_pass += 1
    ...
    def update_passengers(self):
        ...
        for pid in self.passengers.keys():
            if self.passengers[pid].status > 1 or self.passengers[pid].waiting_time >= self.passengers[
                pid].max_tolerate_delay:  # remove passengers after 10 ticks.
    ...
    def dispatch(self, vehicles, current_time):
        ...
        for vehicle in vehicles.values():
            # for test mask action_id as stay still
            ...
            if offduty:
                ...
                trajectory, trip_time, _ = self.routing_engine.route_hex([vehicle.get_location(), target])[0]
                # trajectory is list of coords; trip_time is in seconds.
                if charge_flag == 0:
                    vehicle.state.current_hex = target_hex_id
                    vehicle.cruise(trajectory['coordinates'], trip_time, current_time)  # check if stay still
                else:
                    cid = self.nearest_cs[action_id - DIM_OF_RELOCATION]
                    vehicle.state.current_hex = self.charging_hexes[cid]
                    vehicle.head_for_charging_station(trajectory['coordinates'], trip_time, cid, target)
