...
class hex_zone:
    ...
    def predict_best_action(self, vehicle_id, vehicle,current_time):
    ...
    def get_dispatch_decisions(self, tbd_vehicles,current_time):
        ...
        for vid in tbd_vehicles:
            vehicle = tbd_vehicles[vid]
            # Get best action for this vehicle and whether it will be offduty or not
            a_id, offduty = self.predict_best_action(vehicle.state.vehicle_id, vehicle,current_time)
            if offduty:
                ...
                target, cache_key, charge_flag, target_hex_id = self.convert_action_to_destination(vehicle, a_id)
                ...
                if cache_key is None:
                    ...
                    command = self.create_dispatch_dict(vehicle_id=vehicle.state.vehicle_id, cache_key=cache_key,action=a_id,charge_flag =charge_flag, dest_hex_id =target_hex_id)
