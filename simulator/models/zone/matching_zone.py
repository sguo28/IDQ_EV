...
class matching_zone(object):
    ...
    def match_requests(self, current_time, vehicles, requests):
        ...
        if n_vehicles == 0:
            return match_list
        v_latlon = [[vehicles[veh].state.lat,vehicles[veh].state.lon] for veh in vehicles]# vehicles[["lat", "lon","hex_id"]]
        v_hex_id = [vehicles[veh].state.hex_id for veh in vehicles]
        V = defaultdict(list)
        vid2coord = {}
        for vid, row in enumerate(v_latlon):
            ...
            vid2coord[vid] = coord
            V[coord].append(vid)
        r_latlon = [[requests[order].request.origin_lat, requests[order].request.origin_lon] for order in requests]
        r_hex_id = [requests[order].request.origin_id for order in requests]
        R = defaultdict(list)
