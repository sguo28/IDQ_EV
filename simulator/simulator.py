...
class Simulator(object):
    ...
    def init_zones(self,file_hex,file_charging, trip_file, travel_time_file, n_nearest=NUM_NEAREST_CS):
        ...
        #
        # counter=[matching_zone.remote(idx,hexs) for idx,hexs in zip(matchzones,hex_collects)]
        # print('ray initiaze compelte')
        #
        # for i in range(1000):
        #     t1=time.time()
    ...
    def populate_vehicle(self, vehicle_id, location):
        ...
        self.vehicle_collection[vehicle_id]=Vehicle(VehicleState(vehicle_id,location,idx,type)) #initialize a new vehicle object
        self.hex_zone_collection[idx].add_veh(self.vehicle_collection[vehicle_id]) #append this new available vehicle to the hexagon zone
